...
public class IndexedPage {
    ...
    public IndexedPage(String text) throws IllegalStateException {
        ...
        if (splitWords.length == 0 || splitWords[0].equals("")) {
            throw new IllegalStateException("Il n'y a pas de texte");
        }

        this.words = new String[splitWords.length];
        this.counts = new int[splitWords.length];
        int uniqueIndex = -1;
        for (int i = 0; i < splitWords.length; i++) {
            if (i == 0 || !splitWords[i].equals(splitWords[i - 1])) {
                uniqueIndex++;
                this.words[uniqueIndex] = splitWords[i];
